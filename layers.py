...
class MLPLayers(nn.Module):
    def __init__(
        ...
        self.add_final_dropout = add_final_dropout
        assert self.norm is None or self.norm.lower() in [
            "none",
            "batch_norm",
            "layer_norm",
        ]
        ...
        for idx, (input_size, output_size) in enumerate(
            ...
            if (
                self.norm
                and self.norm != "none"
                and idx != (len(self.layers) - 2)
            ):
                mlp_modules.append(self.build_norm_layer(output_size))
            activation_func = activation_layer(self.activation, output_size)
            if activation_func is not None and idx != (len(self.layers) - 2):
                mlp_modules.append(activation_func)
                mlp_modules.append(nn.Dropout(p=self.dropout))
        if self.add_final_norm and self.norm and self.norm != "none":
            mlp_modules.append(self.build_norm_layer(self.layers[-1]))
        if self.add_final_dropout:
            mlp_modules.append(nn.Dropout(p=self.dropout))
    ...
    def build_norm_layer(self, in_dim):
        if self.norm == "batch_norm":
            return nn.BatchNorm1d(num_features=in_dim)
        else:
